...
contract DAO is IDAO, ERC165, Initializable {
    ...
    function initialize(
        ...
        accessControl = IDAOAccessControl(Clones.clone(accessControlPrototype));
        EXECUTE_ROLE = accessControl.initialize(
            address(this),
            executors,
            roles,
            rolesAdmins,
    ...
    function execute(
        ...
        if (!accessControl.hasRole(EXECUTE_ROLE, msg.sender)) {
            revert Unauthorized(EXECUTE_ROLE, msg.sender);
        }
